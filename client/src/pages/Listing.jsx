...
export default function Listing() {
  ...
  // 3. تحديث الـ Effect ليرسل اللغة الحالية للموقع كمعامل للـ AI ليعود بالبيانات مترجمة تلقائياً
  useEffect(() => {
    const fetchAIValuation = async () => {
      ...
      try {
        ...
        const res = await fetch("/api/listing/evaluate-ai", {
          ...
          body: JSON.stringify({
            ...listing,
            lang: i18n.language // إرسال اللغة الحالية (ar / en) إلى الـ Backend للـ Prompt
          }),
        });
    ...
    fetchAIValuation();
  }, [listing, currentUser, i18n.language]); // إضافة i18n.language للمصفوفة لتحديث بيانات الـ AI فور تغيير لغة الموقع
      ...
      {listing && !loading && !error && (
        <div>
          ...
          <div className="flex flex-col max-w-4xl mx-auto p-6 bg-white my-7 rounded-xl shadow-sm border border-slate-100 gap-5">
            ...
            {/* لوحة التقييم العقاري بالذكاء الاصطناعي (مبنية محلياً لدعم اللغتين بذكاء) */}
            <div
              className="bg-slate-50 p-5 rounded-xl border border-slate-200 shadow-sm my-2"
              dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}
            >
              <div className="flex items-center gap-2 mb-4">
                <span className="text-xl">✨</span>
                <h3 className="text-lg font-bold text-slate-800 font-sans">
                  {t('aiValuation.title', 'AI Valuation & Financial Analysis')}
                </h3>
              </div>
              ...
              {!currentUser ? (
                <div className="text-center p-5 bg-white rounded-lg border border-dashed border-slate-300 shadow-sm">
                  <p className="text-amber-600 font-bold text-sm flex items-center justify-center gap-1 mb-1">
                    {t('aiValuation.lockedTitle', '🔒 Premium Feature')}
                  </p>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    {t('aiValuation.lockedDesc', 'Market analysis and smart valuation are only available for registered members. Please sign in to view fair market estimates and real estate trends.')}
                  </p>
                  <Link
                    to="/sign-in"
                  ...
                  >
                    {t('aiValuation.signInBtn', 'Sign In Now')}
                  </Link>
                </div>
              ) : (
                <>
                  {valLoading && (
                    <p className={`text-slate-500 animate-pulse text-xs ${i18n.language === 'ar' ? 'text-right' : 'text-left'}`}>
                      {t('aiValuation.loading', 'Analyzing neighborhood records and calculating fair property valuation...')}
                    </p>
                  )}

                  {valError && (
                    <p className={`text-red-500 text-xs ${i18n.language === 'ar' ? 'text-right' : 'text-left'}`}>
                      {t('aiValuation.error', 'Could not load AI Valuation at this moment.')}
                    </p>
                  )}

                  {valuation && (
                    <div className={`flex flex-col gap-4 ${i18n.language === 'ar' ? 'text-right' : 'text-left'}`}>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center bg-white p-4 rounded-lg border border-slate-100">
                        <div>
                          <p className="text-xs text-slate-400 mb-1">
                            {t('aiValuation.estimatedMarketValue', 'Estimated Fair Market Value:')}
                          </p>
                        ...
                        <div className={`flex mt-2 md:mt-0 ${i18n.language === 'ar' ? 'md:justify-end' : 'md:justify-start'}`}>
                          <span
                            className={`px-3 py-1.5 rounded-full font-bold text-xs shadow-sm ${
                              valuation.priceStatus === "Good Deal"
                                ? "bg-emerald-100 text-emerald-800 border border-emerald-200"
                                : valuation.priceStatus === "Fair Price"
                                  ? "bg-blue-100 text-blue-800 border border-blue-200"
                                  : valuation.priceStatus === "Overpriced"
                                    ? "bg-rose-100 text-rose-800 border border-rose-200"
                                    : "bg-slate-100 text-slate-800"
                            }`}
                          >
                            {valuation.priceStatus === "Good Deal" && t('aiValuation.goodDeal', '🔥 Good Deal (Below Market Price)')}
                            {valuation.priceStatus === "Fair Price" && t('aiValuation.fairPrice', '✅ Fair Price')}
                            {valuation.priceStatus === "Overpriced" && t('aiValuation.overpriced', '⚠️ Overpriced for this area')}
                            {valuation.priceStatus === "Unknown" && t('aiValuation.unknown', 'ℹ️ No listed price to compare')}
                          </span>
                        </div>
                      </div>

                      {/* حقول الـ AI النصية الديناميكية: تعرض مباشرة (Raw HTML) بدون دالة الترجمة t() حسب الاتفاق */}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white p-4 rounded-lg border border-slate-100">
                          <h4 className="font-bold text-slate-700 text-xs mb-1.5 flex items-center gap-1">
                            {t('aiValuation.trendLabel', '📈 Neighborhood Future (3-5 Years):')}
                          </h4>
                        ...
                        <div className="bg-white p-4 rounded-lg border border-slate-100">
                          <h4 className="font-bold text-slate-700 text-xs mb-1.5 flex items-center gap-1">
                            {t('aiValuation.adviceLabel', '🎯 Investment Advice:')}
                          </h4>
                          <p className="text-slate-600 text-xs leading-relaxed">
            ...
            {/* 🌟 الزر المطلوب تعريبه وترجمته مع معالجة الكلاسات 🌟 */}
            {currentUser && listing.userRef !== currentUser._id && !contact && (
              ...
              >
                {t('listing.contactBtn', 'Contact landlord')}
              </button>
            )}
