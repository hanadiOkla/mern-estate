...
function Profile() {
  ...
  // حالات جديدة لإظهار النوافذ والرسائل المخصصة
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => setUpdateSuccess(false), 4000); // إخفاء الرسالة تلقائياً بعد 4 ثوانٍ
    } catch (error) {
  ...
  // فتح نافذة التأكيد وتخزين الـ id المراد حذفه
  ...
  const handleListingDelete = async () => {
    ...
    try {
      ...
      setShowDeleteModal(false);
      setDeleteSuccess(true); // إظهار رسالة النجاح
      setTimeout(() => setDeleteSuccess(false), 4000); // إخفاء رسالة النجاح بعد 4 ثوانٍ
    ...
    <div className='bg-slate-50/50 min-h-screen py-12 px-4 md:px-8 relative'>
      <div className='max-w-6xl mx-auto flex flex-col gap-8'>
        ...
        <div>
          ...
          <p className='text-sm text-slate-400 mt-1'>Manage your account details and real estate listings.</p>
        </div>
          ...
          <div className='lg:col-span-5 bg-white p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 flex flex-col gap-6'>
            ...
            <div className='flex flex-col items-center gap-3 bg-slate-50/50 py-6 rounded-2xl border border-dashed border-slate-200 relative group'>
              ...
              <div className='text-xs font-semibold text-center mt-1'>
                {fileUploadError ? (
                  <span className='text-red-500 bg-red-50 px-2.5 py-1 rounded-full'>Error Uploading (Max 2MB)</span>
                ) : filePerc > 0 && filePerc < 100 ? (
                  <span className='text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full'>{`Uploading ${filePerc}%`}</span>
                ) : filePerc === 100 ? (
                  <span className='text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full'>Uploaded Successfully!</span>
                ) : (
                  <span className='text-slate-400 font-medium'>Click avatar to update image</span>
                )}
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
              <div className='flex flex-col gap-1.5'>
                <label className='text-xs font-bold text-slate-700 tracking-wide flex items-center gap-1.5 ml-1'>
                  Username
                </label>
                <input
                  type="text" 
            ...
            <div className='flex justify-between items-center border-t border-slate-100 pt-5 text-sm font-semibold'>
              <button onClick={() => setShowDeleteModal(true)} className='text-red-500 hover:text-red-600 transition-colors flex items-center gap-1.5'>
                Delete Account
              </button>
            ...
            {updateSuccess && <div className='bg-emerald-50 border border-emerald-100 text-emerald-600 px-4 py-2.5 rounded-xl text-xs font-medium text-center mt-2'>Profile updated successfully!</div>}
            
            {/* رسالة نجاح الحذف بتصميم مطابق للـ updateSuccess */}
            {deleteSuccess && <div className='bg-rose-50 border border-rose-100 text-rose-600 px-4 py-2.5 rounded-xl text-xs font-medium text-center mt-2'>Listing deleted successfully!</div>}
          </div>
            ...
            <div className='bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 flex flex-col gap-5 min-h-[300px] justify-center'>
              ...
              {userListings.length === 0 ? (
                <div className='text-center py-8 flex flex-col items-center gap-4'>
                  ...
                  <div>
                    <h3 className='font-bold text-slate-800 text-lg'>Your Property Listings</h3>
                    <p className='text-slate-400 text-xs max-w-xs mx-auto mt-1'>Click the button below to retrieve and manage all properties listed under your account.</p>
                  </div>
                  <button 
                  ...
                  {showListingsError && <p className='text-red-500 text-xs font-medium mt-2'>Error loading listings. Please try again.</p>}
                </div>
              ) : (
                <div className='flex flex-col gap-4'>
                  <div className='flex items-center justify-between border-b border-slate-100 pb-3'>
                    <h2 className='text-lg font-bold text-slate-900'>Your Listed Properties ({userListings.length})</h2>
                    <button onClick={handleShowListings} className='text-xs font-bold text-blue-600 hover:underline'>Refresh</button>
                  </div>
                      ...
                      <div key={listing._id} className="border border-slate-100 rounded-2xl p-3 flex justify-between items-center gap-4 hover:shadow-md hover:border-slate-200/80 transition-all duration-200 bg-slate-50/30">
                        ...
                        <div className="flex items-center gap-2">
                          <Link to={`/update-listing/${listing._id}`}>
                            <button className='bg-slate-100 text-slate-700 font-bold text-xs px-3 py-2 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors'>Edit</button>
                          </Link>
                          <button onClick={() => confirmListingDelete(listing._id)} className='bg-rose-50 text-rose-600 font-bold text-xs px-3 py-2 rounded-lg hover:bg-rose-600 hover:text-white transition-colors'>Delete</button>
                        </div>
                      </div>            
                    ))}
                  </div>
      ...
      {/* نافذة مودال منبثقة مخصصة ومطابقة لستايل التصميم بدلاً من الـ alert الافتراضي */}
      {showDeleteModal && (
        <div className='fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all animate-fadeIn'>
          <div className='bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-100 flex flex-col gap-4 scale-100 transition-transform'>
            ...
            <div className='flex justify-end gap-3 font-semibold text-xs uppercase pt-2'>
              <button 
