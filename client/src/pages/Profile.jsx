...
import { 
  FaClock, FaCheckCircle, FaTimesCircle, FaEdit, FaTrash, 
  FaCamera, FaUser, FaEnvelope, FaLock, FaBuilding, FaPlus, FaEye
} from 'react-icons/fa';
...
function Profile() {
  ...
  const handleFileUpdate = (fileToUpload) => {
    ...
    uploadTask.on(
      ...
      () => {
        showToast(t('profile.upload_error', 'Upload error (max 2MB)'), 'error');
      },
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          setFormData((prev) => ({ ...prev, avatar: downloadURL }));
          showToast(t('profile.upload_success', 'Uploaded successfully!'), 'success');
        });
      }
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // تصفية البيانات الفارغة قبل الإرسال
      ...
      showToast(t('profile.update_success_msg', 'Profile updated successfully!'), 'success');
    } catch (error) {
  ...
  const handleSignOut = async () => {
    try {
      ...
      showToast(t('profile.sign_out', 'Signed out successfully'), 'success');
    } catch (error) {
  ...
  const handleShowListings = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success === false) {
        showToast(t('profile.listings_error', 'Error loading listings'), 'error');
  ...
  const handleListingDelete = async () => {
    ...
    try {
      ...
      setShowDeleteModal(false);
      showToast(t('profile.delete_listing_success', 'Listing deleted successfully!'), 'success');
  ...
  const filteredListings = userListings.filter((listing) => {
    if (activeTab === 'active') return listing.status === 'active' || listing.isApproved === true;
    if (activeTab === 'pending') return listing.status === 'pending_approval' || (!listing.status && listing.isApproved === false);
    ...
    if (listing.status === 'active' || listing.isApproved === true) {
      return (
        <span className='inline-flex items-center gap-1.5 bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 text-xs font-semibold px-3 py-1 rounded-full backdrop-blur-sm'>
          <FaCheckCircle className='text-[11px]' /> {t('profile.status_active', 'Approved')}
        </span>
      );
    }
    if (listing.status === 'rejected') {
      return (
        <span className='inline-flex items-center gap-1.5 bg-rose-500/10 text-rose-600 border border-rose-500/20 text-xs font-semibold px-3 py-1 rounded-full backdrop-blur-sm'>
          <FaTimesCircle className='text-[11px]' /> {t('profile.status_rejected', 'Rejected')}
        </span>
      );
    }
    return (
      <span className='inline-flex items-center gap-1.5 bg-amber-500/10 text-amber-600 border border-amber-500/20 text-xs font-semibold px-3 py-1 rounded-full backdrop-blur-sm'>
        <FaClock className='text-[11px]' /> {t('profile.status_pending', 'Pending')}
      </span>
    );
  };
  ...
  const stats = {
    ...
    active: userListings.filter((i) => i.status === 'active' || i.isApproved === true).length,
    pending: userListings.filter((i) => i.status === 'pending_approval' || (!i.status && i.isApproved === false)).length,
    rejected: userListings.filter((i) => i.status === 'rejected').length,
  };

  return (
    <div className='bg-slate-50/60 min-h-screen py-10 px-4 md:px-8 font-sans antialiased' dir={isRtl ? 'rtl' : 'ltr'}>
      {/* تضمين مكون الـ Toast المخصص */}
      {toastInfo.show && (
        <Toast 
      ...
      <div className='max-w-6xl mx-auto flex flex-col gap-8'>
        ...
        <div className='bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 rounded-3xl p-6 md:p-8 text-white shadow-2xl shadow-slate-900/10 relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-6'>
          <div className={`absolute ${isRtl ? '-left-10' : '-right-10'} -bottom-10 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none`} />
          
          <div className='z-10'>
            <h1 className='text-3xl font-extrabold tracking-tight'>{t('profile.title', 'Profile Settings')}</h1>
            <p className='text-slate-400 text-sm mt-1'>{t('profile.subtitle', 'Manage your account details and real estate listings.')}</p>
          </div>
          ...
          <div className='z-10 flex items-center gap-3 bg-white/10 backdrop-blur-md p-2 rounded-2xl border border-white/10 w-full md:w-auto justify-around'>
            <div className='px-4 text-center'>
              <span className='block text-2xl font-extrabold text-indigo-400'>{stats.all}</span>
              <span className='text-[11px] text-slate-300 font-medium'>{t('profile.tab_all', 'All')}</span>
            </div>
            <div className='w-px h-8 bg-white/15' />
            <div className='px-4 text-center'>
              <span className='block text-2xl font-extrabold text-emerald-400'>{stats.active}</span>
              <span className='text-[11px] text-slate-300 font-medium'>{t('profile.tab_active', 'Approved')}</span>
            </div>
            <div className='w-px h-8 bg-white/15' />
            <div className='px-4 text-center'>
              <span className='block text-2xl font-extrabold text-amber-400'>{stats.pending}</span>
              <span className='text-[11px] text-slate-300 font-medium'>{t('profile.tab_pending', 'Pending')}</span>
            </div>
          </div>
        </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-12 gap-8 items-start'>
          ...
          <div className='lg:col-span-5 bg-white p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col gap-6 relative'>
            ...
            <div className='flex flex-col items-center gap-3 relative'>
              ...
              <div className='relative group cursor-pointer' onClick={() => fileRef.current.click()}>
                <div className='w-28 h-28 rounded-full overflow-hidden shadow-xl border-4 border-white ring-4 ring-indigo-50/80 transition-all duration-300 group-hover:ring-indigo-200'>
                  <img src={formData.avatar || currentUser?.avatar} alt="profile" className='w-full h-full object-cover group-hover:scale-110 transition-transform duration-500' />
                </div>
                <div className={`absolute bottom-1 ${isRtl ? 'left-1' : 'right-1'} bg-indigo-600 text-white p-2 rounded-full shadow-lg group-hover:bg-indigo-700 transition-colors`}>
              ...
              <div className='text-xs font-semibold text-center'>
                {filePerc > 0 && filePerc < 100 ? (
                  <span className='text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100'>{`${t('profile.uploading', 'Uploading')} ${filePerc}%`}</span>
                ) : (
                  <span className='text-slate-400 font-medium text-[11px]'>{t('profile.upload_hint', 'Click avatar to update image')}</span>
                )}
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col gap-4.5'>
              ...
              <div className='flex flex-col gap-1.5'>
                <label className='text-xs font-bold text-slate-700 tracking-wide flex items-center gap-1.5'>
                  ...
                  <span>{t('profile.password', 'Password')}</span>
                </label>
                <input 
                  type="password" 
                  placeholder="••••••••" 
              ...
              <div className='flex flex-col gap-3 mt-2'>
                ...
                >
                  {loading ? t('profile.saving', 'Saving...') : t('profile.update_btn', 'Update Settings')}
                </button>
                ...
                >
                  ...
                  <span>{t('profile.create_btn', 'Create New Listing')}</span>
                </Link>
              </div>
            </form>
            ...
            <div className='flex justify-between items-center border-t border-slate-100 pt-5 text-xs font-bold'>
              <button 
                onClick={() => { setListingToDelete(null); setShowDeleteModal(true); }} 
            ...
            <div className='bg-white p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col gap-6 min-h-[450px]'>
              ...
              <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-5'>
                <h2 className='text-xl font-bold text-slate-900 flex items-center gap-2'>
                  ...
                  <span>{t('profile.listings_title', 'My Listings')}</span>
                </h2>

                <div className='flex items-center gap-1.5 bg-slate-100/80 p-1.5 rounded-2xl w-full sm:w-auto overflow-x-auto'>
                  {[
                    { key: 'all', label: t('profile.tab_all', 'All'), count: stats.all },
                    { key: 'active', label: t('profile.tab_active', 'Approved'), count: stats.active },
                    { key: 'pending', label: t('profile.tab_pending', 'Pending'), count: stats.pending },
                    { key: 'rejected', label: t('profile.tab_rejected', 'Rejected'), count: stats.rejected },
                  ].map((tab) => (
                    <button
                      key={tab.key}
              ...
              {loadingListings ? (
                <div className='flex flex-col gap-4 animate-pulse'>
                  {[1, 2, 3].map((i) => (
                    <div key={i} className='border border-slate-100 rounded-2xl p-4 flex items-center justify-between gap-4'>
                ...
                <div className='text-center py-16 flex flex-col items-center justify-center gap-3 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200'>
                  ...
                  <p className='text-slate-400 font-semibold text-sm'>
                    {t('profile.no_listings', 'No listings found in this category.')}
                  </p>
                </div>
              ) : (
                  ...
                  {filteredListings.map((listing) => (
                    ...
                    >
                      <div className='flex items-center gap-4 min-w-0 w-full sm:w-auto'>
                        <Link to={`/listing/${listing._id}`} className='shrink-0 overflow-hidden rounded-xl relative'>
                          <img 
                        ...
                        <div className='flex flex-col gap-1.5 min-w-0 flex-1'>
                          <Link to={`/listing/${listing._id}`} className='text-base font-bold text-slate-800 hover:text-indigo-600 transition-colors truncate'>
                            {listing.name}
                          </Link>
                          {listing.address && (
                          ...
                          <div className='flex items-center gap-3 mt-1'>
                            ...
                            {listing.views !== undefined && (
                              <span className='text-[11px] text-slate-400 flex items-center gap-1 font-medium'>
                                <FaEye className='text-slate-300' /> {listing.views}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100 shrink-0">
                        ...
                        <button 
                          onClick={() => { setListingToDelete(listing._id); setShowDeleteModal(true); }} 
                        ...
                        >
                          ...
                          <span>{t('profile.delete', 'Delete')}</span>
                        </button>
                      </div>
                    </div>
                  ))}
      ...
      {showDeleteModal && (
        <div className='fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all'>
          <div className='bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl border border-slate-100 animate-in fade-in zoom-in duration-200'>
            <h3 className='font-extrabold text-xl text-slate-900'>{t('profile.modal_title', 'Confirm Deletion')}</h3>
            <p className='text-sm text-slate-500 my-4 leading-relaxed'>
              {listingToDelete ? t('profile.modal_desc_listing', 'Are you sure you want to delete this listing? This action cannot be undone.') : t('profile.modal_desc_account', 'Are you sure you want to delete your account? All data will be permanently removed.')}
            </p>
            <div className='flex justify-end gap-3 mt-6'>
              ...
              >
                {t('profile.modal_cancel', 'Cancel')}
              </button>
              <button 
                onClick={listingToDelete ? handleListingDelete : handleDeleteUser} 
              ...
              >
                {t('profile.modal_confirm', 'Yes, Delete')}
              </button>
            </div>
          </div>
        </div>
