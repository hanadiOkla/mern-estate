    ...
    <header className="bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-slate-100 transition-all">
      <div className="flex justify-between items-center max-w-7xl mx-auto p-4 px-6 gap-4">
        ...
        <form
          ...
          className="hidden md:flex bg-slate-50 border border-slate-200/80 p-2.5 px-4 rounded-full items-center w-full max-w-xs md:max-w-md transition-all focus-within:border-blue-500 focus-within:bg-white focus-within:shadow-sm"
        >
          <input
            type="text"
        ...
        <ul className="hidden lg:flex gap-6 items-center font-medium text-sm text-slate-600">
          ...
          {/* 👑 رابط الأدمن للشاشات الكبيرة */}
          {currentUser && currentUser.role === "admin" && (
            <Link to="/admin-dashboard">
              <li className="text-amber-600 font-bold hover:text-amber-700 bg-amber-50 px-3 py-1 rounded-full border border-amber-200 transition-all">
                {t('admin.dashboard_nav')}
              </li>
            </Link>
          )}
        ...
        {/* 4. البروفايل وزر المنيو */}
        <div className="flex items-center gap-4">
          <Link to="/profile" className="flex items-center">
            {currentUser ? (
              <img
                className="rounded-full h-8 w-8 object-cover border-2 border-blue-500/20 hover:border-blue-500 transition-all"
      ...
      <div
        className={`lg:hidden fixed inset-x-0 top-[73px] bg-white border-b border-slate-200 p-6 transition-all duration-300 ease-in-out z-40 ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-5 pointer-events-none"}`}
      >
        <div className="flex flex-col gap-5">
          <form
            onSubmit={handelSubmit}
            className="md:hidden bg-slate-50 border border-slate-200 p-2.5 px-4 rounded-full flex items-center w-full"
          >
            <input
              type="text"
              placeholder={t("search_placeholder")}
          ...
          <ul className="flex flex-col gap-4 font-semibold text-base text-slate-700">
            ...
            {/* 👑 رابط الأدمن لقائمة الموبايل */}
            {currentUser && currentUser.role === "admin" && (
              <Link to="/admin-dashboard" onClick={() => setIsOpen(false)}>
                <li className="py-2 text-amber-600 font-bold border-b border-slate-50 flex items-center justify-between">
                  {/* 💡 التعديل هنا ليدعم الترجمة */}
                  <span>{t("admin.dashboard_nav")}</span>
            ...
            {!currentUser && (
              <Link
                ...
                className="md:hidden"
              >
                <li className="py-2 text-blue-600 font-bold">{t("sign_in")}</li>
              </Link>
            )}
          </ul>
