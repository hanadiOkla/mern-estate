...
export default function Contact({ listing }) {
  ...
  return (
    <>
      {landlord && (
        <div className='bg-white p-6 md:p-8 rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 flex flex-col gap-5 mt-6 animate-fadeIn'>
          ...
          <div>
            ...
            <p className='text-xs text-slate-400 mt-1 leading-relaxed'>
              Send a direct email to <span className='font-semibold text-slate-700'>{landlord.username}</span> regarding <span className='font-semibold text-blue-600'>{listing.name}</span>.
            </p>
          </div>

          {/* حقل كتابة الرسالة */}
          <div className='flex flex-col gap-1.5'>
            <label htmlFor="message" className='text-xs font-bold text-slate-700 tracking-wide ml-1'>
              Your Message
            </label>
            <textarea 
              name="message" 
